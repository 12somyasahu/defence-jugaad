...
const THREAT_ICONS: Dictionary = {
	"NORTH": preload("res://assets/ui/threat/north.png"),
	"SOUTH": preload("res://assets/ui/threat/south.png"),
	"EAST": preload("res://assets/ui/threat/east.png"),
	"WEST": preload("res://assets/ui/threat/west.png"),
}
...
func _ready() -> void:
	...
	_badge.texture = THREAT_ICONS.get(route_name.to_upper(), null)
	if _badge.texture != null:
		var tex_size: float = maxf(_badge.texture.get_width(), _badge.texture.get_height())
		_badge.scale = Vector2.ONE * (44.0 / tex_size)
	add_child(_badge)
	set_active(false)
...
func place_on(bounds: Rect2) -> void:
	...
	if _badge:
		_badge.position = Vector2.ZERO
	var label_size: Vector2 = $Label.size
	if absf(direction.x) > absf(direction.y):
		# East/West: label on the arena side of the arrow.
		$Label.position = -direction * (52.0 + label_size.x * 0.5) - label_size * 0.5
	else:
...
func set_blocked(value: bool) -> void:
	...
	if _badge:
		_badge.modulate = Color(0.45, 0.45, 0.45, 0.8) if blocked else Color.WHITE
	queue_redraw()
...
func _draw() -> void:
	...
	# Inward guide chevrons past the badge
	var tip2: Vector2 = inward * 46.0
	var side2: Vector2 = direction.orthogonal() * 12.0
	draw_polyline(PackedVector2Array([tip2 + -side2, tip2 + inward * 10.0, tip2 + side2]), Color(color, 0.75), 2.5)
